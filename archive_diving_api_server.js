...
function makeToken(payload) {
  ...
  const sig = crypto.createHmac('sha256', JWT_SECRET).update(enc).digest('hex');
  return enc + '.' + sig;
}
function clean(obj) {
  const r = {};
...
input,textarea,select{width:100%;padding:9px 12px;border:1px solid #ddd;border-radius:8px;font-size:14px}
input:focus,textarea:focus{outline:none;border-color:#006994}
...
<div class="tab-content active" id="tab-hero">
  <div class="panel">
    ...
    <button class="btn" onclick="saveHero('zh')">保存</button> <span class="msg" id="hmsg"></span>
  </div>
  <div class="panel">
    <h2>Hero（英文）</h2>
    <div class="grid">
    ...
    <button class="btn" onclick="saveHero('en')">保存</button> <span class="msg" id="hemsg"></span>
  </div>
</div>
...
<div class="tab-content" id="tab-services">
  <div class="panel">
    ...
    <div class="grid">
      ...
      <div><label>描述</label><input id="svc_desc" placeholder="简短描述"></div>
    </div>
    <button class="btn btn-green" onclick="addSvc()">添加</button> <span class="msg" id="svcmsg"></span>
  </div>
</div>
...
<div class="tab-content" id="tab-courses">
  <div class="panel">
    <h2>课程列表</h2>
    <table><thead><tr><th>名称</th><th>标签</th><th>价格</th><th>天数</th><th>操作</th></tr></thead><tbody id="crs_list"></tbody></table>
    <h3>新增课程</h3>
    <div class="grid">
      ...
      <div style="grid-column:1/-1"><label>描述</label><input id="crs_desc" placeholder="课程描述"></div>
    </div>
    <button class="btn btn-green" onclick="addCrs()">添加课程</button> <span class="msg" id="crsmsg"></span>
  </div>
</div>
...
<div class="tab-content" id="tab-destinations">
  <div class="panel">
    ...
    <button class="btn btn-green" onclick="addDst()">添加</button> <span class="msg" id="dstmsg"></span>
  </div>
</div>
...
<div class="tab-content" id="tab-nav">
  <div class="panel">
    ...
    <button class="btn btn-green" onclick="addNav()">添加</button> <span class="msg" id="navmsg"></span>
  </div>
</div>
...
<div class="tab-content" id="tab-pwd">
  <div class="panel">
    <h2>账号信息</h2>
    <p style="color:#888;font-size:13px">admin@imperialdiving.com / admin123</p>
    <h3>改密码</h3>
    <div><label>新密码</label><input type="password" id="npwd" placeholder="留空不改"></div>
    <button class="btn btn-green" onclick="chPwd()">确认修改</button> <span class="msg" id="pwdmsg"></span>
  </div>
</div>

<script>
function showTab(id){document.querySelectorAll('.tab').forEach(t=>t.classList.remove('active'));document.querySelectorAll('.tab-content').forEach(c=>c.classList.remove('active'));document.getElementById('tab-'+id).classList.add('active');document.querySelector('[onclick="showTab(\''+id+'\')"]').classList.add('active');}
async function load(){const h=await fetch('/api/admin/heroes').then(r=>r.json());['zh','en'].forEach(l=>{const s=h[l]||{};['title','subtitle','cta_text','cta_link','cta_secondary_text','cta_secondary_link'].forEach(k=>{const el=document.getElementById('h_'+l+'_'+k.replace('_',''));if(el)el.value=s[k]||'';});});loadSvcs();loadCrss();loadDsts();loadNav();}
async function saveHero(l){const d={};['title','subtitle','cta_text','cta_link','cta_secondary_text','cta_secondary_link'].forEach(k=>{const el=document.getElementById('h_'+l+'_'+k.replace('_',''));if(el)d[k]=el.value;});const r=await fetch('/api/admin/heroes/'+l,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(d)});const m=document.getElementById(l==='zh'?'hmsg':'hemsg');m.textContent=r.ok?'✅ 已保存':'❌ 保存失败';m.className=r.ok?'msg ok':'msg err';}
async function loadSvcs(){const r=await fetch('/api/admin/services').then(r=>r.json());document.getElementById('svc_list').innerHTML=(r||[]).map(s=>'<div class="item"><div class="item-info"><h4>'+s.feature_title+'</h4><p>'+(s.feature_description||'')+'</p></div><button class="btn btn-red" onclick="delSvc('+s.id+')">删除</button></div>').join('')||'<p style="color:#888;font-size:13px">暂无数据</p>';}
async function addSvc(){const d={feature_title:document.getElementById('svc_name').value,feature_description:document.getElementById('svc_desc').value,feature_lang:'zh'};const r=await fetch('/api/admin/services',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(d)});const m=document.getElementById('svcmsg');if(r.ok){m.textContent='✅ 已添加';m.className='msg ok';document.getElementById('svc_name').value='';document.getElementById('svc_desc').value='';loadSvcs();}else{m.textContent='❌ 添加失败';m.className='msg err';}}
async function delSvc(id){if(!confirm('确认删除?'))return;await fetch('/api/admin/services/'+id,{method:'DELETE'});loadSvcs();}
async function loadCrss(){const r=await fetch('/api/admin/courses').then(r=>r.json());document.getElementById('crs_list').innerHTML=(r||[]).map(c=>'<tr><td>'+c.course_name+'</td><td>'+c.course_tag+'</td><td>'+c.course_price+'</td><td>'+c.course_days+'</td><td><button class="btn btn-red" onclick="delCrs(\''+c.id+'\')">删除</button></td></tr>').join('')||'<tr><td colspan="5" style="color:#888">暂无数据</td></tr>';}
async function addCrs(){const d={course_name:document.getElementById('crs_name').value,course_tag:document.getElementById('crs_tag').value,course_price:document.getElementById('crs_price').value,course_days:document.getElementById('crs_days').value,course_description:document.getElementById('crs_desc').value,course_lang:'zh'};const r=await fetch('/api/admin/courses',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(d)});const m=document.getElementById('crsmsg');if(r.ok){m.textContent='✅ 已添加';m.className='msg ok';['crs_name','crs_tag','crs_price','crs_days','crs_desc'].forEach(id=>document.getElementById(id).value='');loadCrss();}else{m.textContent='❌ 添加失败';m.className='msg err';}}
async function delCrs(id){if(!confirm('确认删除?'))return;await fetch('/api/admin/courses/'+id,{method:'DELETE'});loadCrss();}
async function loadDsts(){const r=await fetch('/api/admin/destinations').then(r=>r.json());document.getElementById('dst_list').innerHTML=(r||[]).map(d=>'<tr><td>'+d.dest_name+'</td><td>'+d.dest_tag+'</td><td><code>'+d.dest_code+'</code></td><td><button class="btn btn-red" onclick="delDst(\''+d.id+'\')">删除</button></td></tr>').join('')||'<tr><td colspan="4" style="color:#888">暂无数据</td></tr>';}
async function addDst(){const d={dest_name:document.getElementById('dst_name').value,dest_tag:document.getElementById('dst_tag').value,dest_code:document.getElementById('dst_code').value,dest_lang:'zh'};const r=await fetch('/api/admin/destinations',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(d)});const m=document.getElementById('dstmsg');if(r.ok){m.textContent='✅ 已添加';m.className='msg ok';['dst_name','dst_tag','dst_code'].forEach(id=>document.getElementById(id).value='');loadDsts();}else{m.textContent='❌ 添加失败';m.className='msg err';}}
async function delDst(id){if(!confirm('确认删除?'))return;await fetch('/api/admin/destinations/'+id,{method:'DELETE'});loadDsts();}
async function loadNav(){const r=await fetch('/api/admin/navigation').then(r=>r.json());document.getElementById('nav_list').innerHTML=(r||[]).map(n=>'<div class="item"><div class="item-info"><h4>'+n.nav_name+'</h4><p>'+n.nav_url+'</p></div><button class="btn btn-red" onclick="delNav(\''+n.id+'\')">删除</button></div>').join('')||'<p style="color:#888;font-size:13px">暂无数据</p>';}
async function addNav(){const d={nav_name:document.getElementById('nav_name').value,nav_url:document.getElementById('nav_url').value,nav_lang:'zh'};const r=await fetch('/api/admin/navigation',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(d)});const m=document.getElementById('navmsg');if(r.ok){m.textContent='✅ 已添加';m.className='msg ok';document.getElementById('nav_name').value='';document.getElementById('nav_url').value='';loadNav();}else{m.textContent='❌ 添加失败';m.className='msg err';}}
async function delNav(id){if(!confirm('确认删除?'))return;await fetch('/api/admin/navigation/'+id,{method:'DELETE'});loadNav();}
...
const server = http.createServer(async (req, res) => {
  ...
  if (path.startsWith('/auth/')) {
    ...
    if (path === '/auth/login' && method === 'POST') {
      ...
      return sendJSON(res, 200, { token, user: { id: user.id, email: user.email, name: user.name, role: user.role } });
    }
  ...
  // Public: Health
  if (path === '/api/health' && method === 'GET') return sendJSON(res, 200, { status: 'ok' });

  // Public: Heroes
  if (path === '/api/heroes' && method === 'GET') {
    const lang = query.lang || 'zh';
    return sendJSON(res, 200, data.heroes[lang] || data.heroes.zh);
  }

  // Public: Columns (legacy JSON)
  ...
  // Public: Services (MySQL page_feature)
  if (path === '/api/services' && method === 'GET') {
    try {
      const lang = query.lang || 'zh';
      const rows = await query('SELECT * FROM page_feature WHERE feature_lang = ? ORDER BY feature_sort ASC', [lang]);
      return sendJSON(res, 200, rows);
    } catch(e) { return sendJSON(res, 500, { error: e.message }); }
  }

  // Public: Courses (MySQL)
  if (path === '/api/courses' && method === 'GET') {
    try {
      const lang = query.lang || 'zh';
      const rows = await query('SELECT * FROM courses WHERE course_lang = ? ORDER BY course_sort ASC', [lang]);
      return sendJSON(res, 200, rows);
    } catch(e) { return sendJSON(res, 500, { error: e.message }); }
  }

  // Public: Destinations (MySQL)
  if (path === '/api/destinations' && method === 'GET') {
    try {
      const lang = query.lang || 'zh';
      const rows = await query('SELECT * FROM page_destination WHERE dest_lang = ? AND enabled = 1 ORDER BY dest_order ASC', [lang]);
      return sendJSON(res, 200, rows);
    } catch(e) { return sendJSON(res, 500, { error: e.message }); }
  }

  // Public: Navigation (MySQL)
  if (path === '/api/navigation' && method === 'GET') {
    try {
      const lang = query.lang || 'zh';
      const rows = await query('SELECT * FROM page_navigation WHERE nav_lang = ? AND nav_status = 1 ORDER BY nav_order ASC', [lang]);
      return sendJSON(res, 200, rows);
    } catch(e) { return sendJSON(res, 500, { error: e.message }); }
  }

  // Admin: Heroes
  if (path === '/api/admin/heroes' && method === 'GET') {
    if (!adminAuth()) return;
  ...
  if (heroMatch && method === 'PUT') {
    if (!adminAuth()) return;
    const lang = heroMatch[1];
    const body = await parseBody(req);
    if (!data.heroes[lang]) data.heroes[lang] = {};
    data.heroes[lang] = { ...data.heroes[lang], ...clean(body), updated_at: new Date().toISOString() };
    fs.writeFileSync(DATA_FILE, JSON.stringify(data, null, 2));
  ...
  if (path === '/api/admin/services' && method === 'GET') {
    if (!adminAuth()) return;
    try { return sendJSON(res, 200, await query('SELECT * FROM page_feature ORDER BY feature_sort ASC')); }
    catch(e) { return sendJSON(res, 500, { error: e.message }); }
  }
  if (path === '/api/admin/services' && method === 'POST') {
    if (!adminAuth()) return;
    try {
      const body = clean(await parseBody(req));
      const id = id8();
      await query('INSERT INTO page_feature (id, feature_lang, feature_title, feature_description, feature_sort) VALUES (?,?,?,?,?)',
        [id, body.feature_lang || 'zh', body.feature_title, body.feature_description, body.feature_sort || 0]);
  ...
  if (svcDel && method === 'DELETE') {
    if (!adminAuth()) return;
    try { await query('DELETE FROM page_feature WHERE id = ?', [svcDel[1]]); return sendJSON(res, 200, { ok: true }); }
    catch(e) { return sendJSON(res, 500, { error: e.message }); }
  }
  ...
  if (path === '/api/admin/courses' && method === 'GET') {
    if (!adminAuth()) return;
    try { return sendJSON(res, 200, await query('SELECT * FROM courses ORDER BY course_sort ASC')); }
    catch(e) { return sendJSON(res, 500, { error: e.message }); }
  }
  if (path === '/api/admin/courses' && method === 'POST') {
    ...
    try {
      const body = clean(await parseBody(req));
      const id = id8();
      await query('INSERT INTO courses (id, course_code, course_lang, course_name, course_tag, course_description, course_price, course_days, course_sort) VALUES (?,?,?,?,?,?,?,?,?)',
        [id, body.course_code || id, body.course_lang || 'zh', body.course_name, body.course_tag, body.course_description, body.course_price, body.course_days, body.course_sort || 0]);
      return sendJSON(res, 201, { id });
    } catch(e) { return sendJSON(res, 500, { error: e.message }); }
  }
  const crsDel = path.match(/^\/api\/admin\/courses\/(\w+)$/);
  if (crsDel && method === 'DELETE') {
    if (!adminAuth()) return;
    try { await query('DELETE FROM courses WHERE id = ?', [crsDel[1]]); return sendJSON(res, 200, { ok: true }); }
    catch(e) { return sendJSON(res, 500, { error: e.message }); }
  }

  // Admin: Destinations CRUD
  if (path === '/api/admin/destinations' && method === 'GET') {
    if (!adminAuth()) return;
    try { return sendJSON(res, 200, await query('SELECT * FROM page_destination ORDER BY dest_order ASC')); }
    catch(e) { return sendJSON(res, 500, { error: e.message }); }
  }
  if (path === '/api/admin/destinations' && method === 'POST') {
    ...
    try {
      const body = clean(await parseBody(req));
      const id = id8();
      await query('INSERT INTO page_destination (id, dest_code, dest_lang, dest_name, dest_tag, dest_order) VALUES (?,?,?,?,?,?)',
        [id, body.dest_code || id, body.dest_lang || 'zh', body.dest_name, body.dest_tag, body.dest_order || 0]);
      return sendJSON(res, 201, { id });
    } catch(e) { return sendJSON(res, 500, { error: e.message }); }
  }
  const dstDel = path.match(/^\/api\/admin\/destinations\/(\w+)$/);
  if (dstDel && method === 'DELETE') {
    if (!adminAuth()) return;
    try { await query('DELETE FROM page_destination WHERE id = ?', [dstDel[1]]); return sendJSON(res, 200, { ok: true }); }
    catch(e) { return sendJSON(res, 500, { error: e.message }); }
  }
  ...
  if (path === '/api/admin/navigation' && method === 'GET') {
    if (!adminAuth()) return;
    try { return sendJSON(res, 200, await query('SELECT * FROM page_navigation ORDER BY nav_order ASC')); }
    catch(e) { return sendJSON(res, 500, { error: e.message }); }
  }
  if (path === '/api/admin/navigation' && method === 'POST') {
    ...
    try {
      const body = clean(await parseBody(req));
      const id = id8();
      await query('INSERT INTO page_navigation (id, nav_name, nav_url, nav_order, nav_lang) VALUES (?,?,?,?,?)',
        [id, body.nav_name, body.nav_url, body.nav_order || 0, body.nav_lang || 'zh']);
      return sendJSON(res, 201, { id });
    } catch(e) { return sendJSON(res, 500, { error: e.message }); }
  }
  const navDel = path.match(/^\/api\/admin\/navigation\/(\w+)$/);
  if (navDel && method === 'DELETE') {
    if (!adminAuth()) return;
    try { await query('DELETE FROM page_navigation WHERE id = ?', [navDel[1]]); return sendJSON(res, 200, { ok: true }); }
    catch(e) { return sendJSON(res, 500, { error: e.message }); }
  }
  ...
  if (path === '/api/admin/password' && method === 'POST') {
    if (!adminAuth()) return;
    const body = await parseBody(req);
    if (!body.password || body.password.length < 6) return sendJSON(res, 400, { error: 'Min 6 chars' });
    data.users[0].password = crypto.createHash('sha256').update(body.password).digest('hex');
    fs.writeFileSync(DATA_FILE, JSON.stringify(data, null, 2));
    return sendJSON(res, 200, { ok: true });
  }
