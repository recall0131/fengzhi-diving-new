    ...
    return new Promise((resolve, reject) => {
      ...
      req.on('end', () => {
        ...
        for (const part of parts) {
          if (!part.includes('filename') || part.includes('--')) continue;
          ...
          if (nameMatch && filenameMatch) {
            ...
            if (!allowedExts.includes(ext)) {
              return reject(new Error('Invalid file type: ' + ext));
            }
            ...
            const fileData = body.substring(0, body.lastIndexOf('\r\n'));
            fs.writeFileSync(filePath, Buffer.from(fileData, 'binary'));
            
            result.files[name] = { path: '/uploads/' + uniqueName, original: filename };
          } else if (nameMatch) {
            const name = nameMatch[1];
            result.fields[name] = body.substring(0, body.lastIndexOf('\r\n'));
          }
        }
  ...
  <div class="panel"><h2>目的地管理</h2>
    <div id="dst_edit_panel" style="display:none;background:#f5f5f5;padding:15px;margin:10px 0;border-radius:8px">
      ...
      <div class="grid">
        ...
        <div><label>图片上传</label><input type="file" id="dst_image_file" accept="image/*" onchange="previewDstImage(this)" style="width:100%;padding:8px;border:1px solid #ddd;border-radius:8px"></div>
        <div style="margin-top:8px"><label>图片路径</label><input id="dst_edit_image" placeholder="/uploads/xxx.jpg" readonly style="background:#f5f5f5"></div>
        <div id="dst_image_preview" style="margin-top:8px"></div>
...
async function loadDsts(){const r=await fetch('/api/admin/destinations',{headers:{'Authorization':'Bearer '+getToken()}}).then(r=>r.json());document.getElementById('dst_list').innerHTML=(r||[]).map(d=>'<tr><td>'+d.dest_name+'</td><td>'+d.dest_tag+'</td><td><code>'+d.dest_code+'</code></td><td><button class="btn" onclick="editDst('+d.id+')"" style="margin-right:4px">编辑</button><button class="btn btn-red" onclick="delDst('+d.id+')"">删除</button></td></tr>').join('')||'<tr><td colspan="4" style="color:#888">暂无数据</td></tr>';}
...
if (path === '/api/health') return sendJSON(res, 200, { status: 'ok' });
  ...
  if (path.match(/^\/api\/admin\/destinations\/(\d+)$/) && method === 'GET') { if (!adminAuth()) return; const id = path.split('/').pop(); try { const rows = await q('SELECT * FROM page_destination WHERE id = ?', [id]); if (rows.length) return sendJSON(res, 200, rows[0]); return sendJSON(res, 404, { error: 'Not found' }); } catch(e) { return sendJSON(res, 500, { error: e.message }); } }
  if (path.match(/^\/api\/admin\/destinations\/(\d+)$/) && method === 'PUT') { if (!adminAuth()) return; const id = path.split('/').pop(); const body = await parseBody(req); const fields = []; const values = []; if (body.dest_tag !== undefined) { fields.push('dest_tag = ?'); values.push(body.dest_tag); } if (body.dest_image !== undefined) { fields.push('dest_image = ?'); values.push(body.dest_image); } if (body.dest_image_alt !== undefined) { fields.push('dest_image_alt = ?'); values.push(body.dest_image_alt); } if (body.dest_description !== undefined) { fields.push('dest_description = ?'); values.push(body.dest_description); } if (body.dest_content !== undefined) { fields.push('dest_content = ?'); values.push(body.dest_content); } if (fields.length === 0) return sendJSON(res, 400, { error: 'No fields to update' }); values.push(id); try { await q('UPDATE page_destination SET ' + fields.join(', ') + ' WHERE id = ?', values); return sendJSON(res, 200, { ok: true }); } catch(e) { return sendJSON(res, 500, { error: e.message }); } }
  if (path.match(/^\/api\/admin\/destinations\/(\d+)$/) && method === 'DELETE') { if (!adminAuth()) return; const id = path.split('/').pop(); try { await q('DELETE FROM page_destination WHERE id = ?', [id]); return sendJSON(res, 200, { ok: true }); } catch(e) { return sendJSON(res, 500, { error: e.message }); } }
